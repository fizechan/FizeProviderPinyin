 ...
 * 本地简易
 */
class Simple extends PinYinHandler
{
    ...
    public function get($words, $delimiter = ' ')
    {
        ...
        $words = self::utf8ToGbk($words);
        $pinyins = [];
        for ($i = 0; $i < strlen($words); $i++) {
            $p = ord(substr($words, $i, 1));
            if ($p > 160) {
                $q = ord(substr($words, ++$i, 1));
                $p = $p * 256 + $q - 65536;
            }
            $pinyin = self::asciiToPinyin($p, $maps);
            $pinyin = Preg::replace("/[^a-z0-9_\-]*/", '', $pinyin);
            $pinyins[] = $pinyin;
        }
    ...
    public function getInitial($words, $delimiter = '')
    {
        ...
        return implode($delimiter, $initials);
    }

    /**
     * ASCII码转拼音
     * @param int $num ASCII码
     * @param array $maps 拼音码表
     * @return string
     */
    private static function asciiToPinyin($num, array $maps)
    {
        if ($num > 0 && $num < 160) {
            return chr($num);
